...
const prepareData = () => {
    let data = {...registrationForm,...addressForm,...vrs_xtd}
    Object.keys(preview_init).map((index) => {
        if(imgFile[index] instanceof File){
            data = {...data,[index]:imgFile[index]}
        }
    })
    return data;
}
const submitVRSForm = async (e) =>  {
    ...
    const  headers = {
        'content-type':'multipart/form-data',
         'Authorization':`Token ${params.get('token')}`
    }
    ...
    await axios({
        method: 'PATCH',
        url:`${import.meta.env.VITE_API_URL}/single-vrs/${params.get('id')}/`,
        data: data,
        headers:headers
        }).then((resp)=>{
            
            if(resp.status == 200 && resp.statusText=='OK'){
                toast.success("Form Updated Successfully ... Pending For Verification");
                navigate("/accounts");
            }
    }).catch((err)=>{
...
// extended infromation of VRS form 
const[vrs_xtd,setExtended] = useState({})


// popluating form with DB data
const populateData = (data) => {
    // set all data to the forms
        Object.keys(vrs_reg_init).map((index) => {
            vrs_reg_init[index] = data[index];    
        })

        setRegistrationForm(vrs_reg_init);
        setCurrentClass(vehicle_class[data.vrs_vehicle_cat]);

        Object.keys(permanent_address_init).map((index) => {
            permanent_address_init[index] = data[index];
        })
        setAddressForm(permanent_address_init);

        Object.keys(preview_init).map((index)=>{
            preview_init[index] =`${data[index]}`;
        })
        setPreview(preview_init);
        
        setExtended({...data['vrx_xtd'],status:'pending'});
        
        setData(true);
    }
// axios fetching information for update
        ...
        headers:{
            ...
            if(resp.data.status != 'rejected'){
             ...
             return   
                ...
                toast.info("You can update and submit your data");
            }       
        }).catch((error) =>{
            
            toast.warning("Some error Occurred while fetching information...")
            // navigate("/accounts");
        })
    }
useEffect(() =>{
        ...
        <div className="row">
                <div className="col-12 bg-primary ">
                    <div className="text text-center text-light">
                       <span className="text text-danger me-1">UPDATE</span> SINGLE VEHICLE REGISTRATION /एक्कल सवारी दर्ता
                    </div>
                </div>
        </div>
            ...
            <div className="shadow shadow-sm my-2">
                <div className="row m-2 p-2">
                    ...
                    <div className="col-7 my-2">
                        ...
                        <input accept="image/*" name="citizen_doc_img" className={`form-control ${error.citizen_doc_img.has_error?"is-invalid":error.citizen_doc_img.valid && "is-valid"}`} type="file" onChange={onSelectImageFile} />
            ...
            <div className="shadow shadow-sm my-2">
                    <div className="row m-2 p-2">
                        ...
                        <div className="col-12 border-md border-danger border my-3">
                            <input required={false} autoComplete="off" className="form-check-input" type="checkbox" name="same_as_permanent" value={same_as_permanent} onInput={()=>{
            ...
            <div className="col-12 my-2">
                {spinner?<ButtonSpinner msg="Update Process Initiating..."/>
